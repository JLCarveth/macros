...
import data from "../FoodData_Central_sr_legacy_food_json_2018-04.json" with { type: "json" };
...
async function main() {
  ...
  try {
    ...
    for (const food of data.SRLegacyFoods) {
      ...
      const sugars = getNutrient("269"); // SR Legacy uses "269" (not "269.3")
    ...
    console.log(`Successfully imported ${count} SR Legacy foods (${skipped} skipped).`);
